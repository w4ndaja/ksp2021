...
export default function Index({ providers, request }) {
    ...
    const edit = e => {
        let { name } = providers.data.find(item => item.id == selectedItem[0] || 0);
        reset();
        setData({ name });
        setFormState('edit');
        Inertia.reload();
        modal.current.show();
        Inertia.reload({ only: ['providers'], });
    };
    ...
    const submitForm = e => {
        ...
        switch (formState) {
            case 'create':
                ...
                break;
            case 'edit': patch(route('providers.update', selectedItem[0])); break;
            case 'delete': _delete(route('providers.destroy', selectedItem[0])); break;
        }
    };
    const destroy = e => {
        e.preventDefault();
        swal('Peringatan', 'Apakah anda yakin ingin menghapus data terpilih?', 'warning', { buttons: ['Tidak', 'Ya'] }).then(e => {
            if (e) {
                _delete(route('providers.destroy', selectedItem[0]));
                setSelectedItem([]);
            };
        });
    };
    ...
    const searchData = useCallback(_.debounce(query => {
        Inertia.reload({ data: { query, page: 1 }, only: ['providers'] });
    }, 300), []);

    const changePerPage = e => {
        const per_page = e.target.value;
        setPerPage(per_page);
        Inertia.reload({ data: { per_page, page: 1 }, only: ['providers'], replace: true });
    };
    return (
        <Card title="Penyedia">
            ...
            <ModalFade title={ `${formState == 'create' ? 'Tambah' : 'Edit'} Penyedia` } ref={ modal }>
                <form onSubmit={ submitForm }>
                    <InputGroup label="Penyedia" value={ data.name } name="name" onChange={ change } />
            ...
            <Table>
                <thead>
                    <tr>
                        <th>{ providers.data.length > 0 && <input type="checkbox" title="Check ALL" onChange={ (e) => setSelectedItem(!e.target.checked ? [] : providers.data.map(item => item.id)) } checked={ providers.data.length == selectedItem.length } /> }</th>
                        <th>Penyedia</th>
                    </tr>
                </thead>
                <tbody>
                    { providers.data.length == 0 && <tr><td className="text-center h1" colSpan={ 999999 }>Data Not Available</td></tr> }
                    { providers.data.map(tableItem => {
                        ...
                        return (
                            <tr key={ tableItem.id } className={ `${selected ? 'bg-soft-primary' : ''}` } onClick={ () => {
                                ...
                                <td className="text-nowrap">{ tableItem.rkn_nama }</td>
                            </tr>
                        );
                    }) }
                </tbody>
            </Table>
            <PaginateLinks links={ providers.links } only={ ['providers'] } />
        </Card>
    );
}
