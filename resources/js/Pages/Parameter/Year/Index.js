...
export default function Index({ years, request }) {
    ...
    const edit = e => {
        let { year } = years.data.find(item => item.id == selectedItem[0] || 0);
        reset();
        setData({ year });
        setFormState('edit');
        Inertia.reload();
        modal.current.show();
        Inertia.reload({ only: ['years'], });
    };
    ...
    const submitForm = e => {
        ...
        switch (formState) {
            case 'create':
                ...
                break;
            case 'edit': patch(route('years.update', selectedItem[0])); break;
            case 'delete': _delete(route('years.destroy', selectedItem[0])); break;
        }
    };
    const destroy = e => {
        e.preventDefault();
        swal('Peringatan', 'Apakah anda yakin ingin menghapus data terpilih?', 'warning', { buttons: ['Tidak', 'Ya'] }).then(e => {
            if (e) {
                _delete(route('years.destroy', selectedItem[0]));
                setSelectedItem([]);
            };
        });
    };
    ...
    const searchData = useCallback(_.debounce(query => {
        Inertia.reload({ data: { query, page: 1 }, only: ['years'] });
    }, 300), []);

    const changePerPage = e => {
        const per_page = e.target.value;
        setPerPage(per_page);
        Inertia.reload({ data: { per_page, page: 1 }, only: ['years'], replace: true });
    };
    return (
        <Card title="Tahun">
            ...
            <ModalFade title={ `${formState == 'create' ? 'Tambah' : 'Edit'} Tahun` } ref={ modal }>
                <form onSubmit={ submitForm }>
                    <InputGroup label="Tahun" value={ data.year } type="number" name="year" onChange={ change } />
            ...
            <Table>
                <thead>
                    <tr>
                        <th>{ years.data.length > 0 && <input type="checkbox" title="Check ALL" onChange={ (e) => setSelectedItem(!e.target.checked ? [] : years.data.map(item => item.id)) } checked={ years.data.length == selectedItem.length } /> }</th>
                        <th>Tahun</th>
                    </tr>
                </thead>
                <tbody>
                    { years.data.length == 0 && <tr><td className="text-center h1" colSpan={ 999999 }>Data Not Available</td></tr> }
                    { years.data.map(tableItem => {
                        ...
                        return (
                            <tr key={ tableItem.id } className={ `${selected ? 'bg-soft-primary' : ''}` } onClick={ () => {
                                ...
                                <td className="text-nowrap">{ tableItem.year }</td>
                            </tr>
                        );
                    }) }
                </tbody>
            </Table>
            <PaginateLinks links={ years.links } only={ ['years'] } />
        </Card>
    );
}
