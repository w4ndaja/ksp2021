...
export default function Index({ regencies, provinces, request }) {
    ...
    const changeProvince = e => {
        ...
        Inertia.reload({ data: { province_id: e.target.value }, only: ['regencies'] });
    };
    const modal = useRef();
    ...
    const edit = e => {
        let { name, id } = regencies.data.find(item => item.id == selectedItem[0] || 0);
        reset();
        setData({ name, id });
        setFormState('edit');
        Inertia.reload();
        modal.current.show();
        Inertia.reload({ only: ['regencies'], });
    };
    ...
    const submitForm = e => {
        ...
        switch (formState) {
            case 'create':
                ...
                break;
            case 'edit': patch(route('regencies.update', selectedItem[0])); break;
            case 'delete': _delete(route('regencies.destroy', selectedItem[0])); break;
        }
    };
    const destroy = e => {
        e.preventDefault();
        swal('Peringatan', 'Apakah anda yakin ingin menghapus data terpilih?', 'warning', { buttons: ['Tidak', 'Ya'] }).then(e => {
            if (e) {
                _delete(route('regencies.destroy', selectedItem[0]));
                setSelectedItem([]);
            };
        });
    };
    ...
    const searchData = useCallback(_.debounce(query => {
        Inertia.reload({ data: { query, page: 1 }, only: ['regencies'] });
    }, 300), []);

    const changePerPage = e => {
        const per_page = e.target.value;
        setPerPage(per_page);
        Inertia.reload({ data: { per_page, page: 1 }, only: ['regencies'], replace: true });
    };
    return (
        <Card title="Kabupaten/Kota">
            ...
            <ModalFade title={ `${formState == 'create' ? 'Tambah' : 'Edit'} Kabupaten Kota` } ref={ modal }>
                <form onSubmit={ submitForm }>
                    <InputGroup label="Kode" value={ data.id } name="id" onChange={ change } />
            ...
            <SelectGroup label="Provinsi" name="province_id" value={ provinceId } data={ provinces.map(item => ({ value: item.id, label: item.name })) } onChange={ changeProvince } />
            <Table>
                <thead>
                    <tr>
                        <th>{ regencies.data.length > 0 && <input type="checkbox" title="Check ALL" onChange={ (e) => setSelectedItem(!e.target.checked ? [] : regencies.data.map(item => item.id)) } checked={ regencies.data.length == selectedItem.length } /> }</th>
                        <th>KODE</th>
                        <th>NAMA</th>
                        <th></th>
                    </tr>
                ...
                <tbody>
                    { regencies.data.length == 0 && <tr><td className="text-center h1" colSpan={ 999999 }>Data Not Available</td></tr> }
                    { regencies.data.map(tableItem => {
            ...
            <PaginateLinks links={ regencies.links } only={ ['regencies'] } />
        </Card>
    );
}
