...
export default function Index({ districs, regencies, request }) {
    ...
    const changeProvince = e => {
        ...
        Inertia.reload({ data: { regency_id: e.target.value }, only: ['districs'] });
    };
    const modal = useRef();
    ...
    const edit = e => {
        let { name, id } = districs.data.find(item => item.id == selectedItem[0] || 0);
        reset();
        setData({ name, id });
        setFormState('edit');
        Inertia.reload();
        modal.current.show();
        Inertia.reload({ only: ['districs'], });
    };
    ...
    const submitForm = e => {
        ...
        switch (formState) {
            case 'create':
                post(route('districs.store'));
                break;
            case 'edit': patch(route('districs.update', selectedItem[0])); break;
            case 'delete': _delete(route('districs.destroy', selectedItem[0])); break;
        }
    };
    const destroy = e => {
        e.preventDefault();
        swal('Peringatan', 'Apakah anda yakin ingin menghapus data terpilih?', 'warning', { buttons: ['Tidak', 'Ya'] }).then(e => {
            if (e) {
                _delete(route('districs.destroy', selectedItem[0]));
                setSelectedItem([]);
            };
        });
    };
    ...
    const searchData = useCallback(_.debounce(query => {
        Inertia.reload({ data: { query, page: 1 }, only: ['districs'] });
    }, 300), []);

    const changePerPage = e => {
        const per_page = e.target.value;
        setPerPage(per_page);
        Inertia.reload({ data: { per_page, page: 1 }, only: ['districs'], replace: true });
    };
    return (
        <Card title="Kecamatan">
            ...
            <ModalFade title={ `${formState == 'create' ? 'Tambah' : 'Edit'} Kecamatan` } ref={ modal }>
            ...
            <SelectGroup label="Provinsi" name="regency_id" value={ regencyId } data={ regencies.map(item => ({ value: item.id, label: item.name })) } onChange={ changeProvince } />
            <Table>
                <thead>
                    <tr>
                        <th>{ districs.data.length > 0 && <input type="checkbox" title="Check ALL" onChange={ (e) => setSelectedItem(!e.target.checked ? [] : districs.data.map(item => item.id)) } checked={ districs.data.length == selectedItem.length } /> }</th>
                        <th>KODE</th>
                        <th>NAMA</th>
                        <th></th>
                    </tr>
                ...
                <tbody>
                    { districs.data.length == 0 && <tr><td className="text-center h1" colSpan={ 999999 }>Data Not Available</td></tr> }
                    { districs.data.map(tableItem => {
            ...
            <PaginateLinks links={ districs.links } only={ ['districs'] } />
        </Card>
    );
}
