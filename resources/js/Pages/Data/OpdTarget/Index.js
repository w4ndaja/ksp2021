...
export default function Index({ opdTargets, years, opdWorkUnits, request }) {
    ...
    const edit = e => {
        let { year_id, opd_work_unit_id, january, february, march, april, mei, june, july, august, september, october, november, december } = opdTargets.data.find(item => item.id == selectedItem[0] || 0);
        reset();
    ...
    const submitForm = e => {
        ...
        switch (formState) {
            case 'create':
                post(route('opd-targets.store'));
                break;
            case 'edit': patch(route('opd-targets.update', selectedItem[0])); break;
            case 'delete': _delete(route('opd-targets.destroy', selectedItem[0])); break;
        }
    };
    const destroy = e => {
        e.preventDefault();
        swal('Peringatan', 'Apakah anda yakin ingin menghapus data terpilih?', 'warning', { buttons: ['Tidak', 'Ya'] }).then(e => {
            if (e) {
                _delete(route('opd-targets.destroy', selectedItem[0]));
                setSelectedItem([]);
            };
        });
    };
    ...
    const searchData = useCallback(_.debounce(query => {
        Inertia.reload({ data: { query, page: 1 }, only: ['opdTargets'] });
    }, 300), []);
    const changePerPage = e => {
        const per_page = e.target.value;
        setPerPage(per_page);
        Inertia.reload({ data: { per_page, page: 1 }, only: ['opdTargets'], replace: true });
    };
    return (
        <Card title="Target OPD">
            ...
            <ModalFade title={ `${formState == 'create' ? 'Tambah' : 'Edit'} Target OPD` } ref={ modal }>
            ...
            <Table>
                <thead>
                    <tr>
                        <th>{ opdTargets.data.length > 0 && <input type="checkbox" title="Check ALL" onChange={ (e) => setSelectedItem(!e.target.checked ? [] : opdTargets.data.map(item => item.id)) } checked={ opdTargets.data.length == selectedItem.length } /> }</th>
                ...
                <tbody>
                    { opdTargets.data.length == 0 && <tr><td className="text-center h1" colSpan={ 999999 }>Data Not Available</td></tr> }
                    { opdTargets.data.map(opdTarget => {
            ...
            <PaginateLinks links={ opdTargets.links } only={ ['opdTargets'] } />
        </Card>
    );
}
