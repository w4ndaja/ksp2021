...
export default function Index({ apbnBudgets, years, opdWorkUnits, request }) {
    ...
    const edit = e => {
        let { year_id, opd_work_unit_id, budget } = apbnBudgets.data.find(item => item.id == selectedItem[0] || 0);
        reset();
    ...
    const submitForm = e => {
        e.preventDefault();
        switch (formState) {
            case 'create': post(route('apbn-budgets.store')); break;
            case 'edit': patch(route('apbn-budgets.update', selectedItem[0])); break;
            case 'delete': _delete(route('apbn-budgets.destroy', selectedItem[0])); break;
        }
    };
    const destroy = e => {
        e.preventDefault();
        swal('Peringatan', 'Apakah anda yakin ingin menghapus data terpilih?', 'warning', { buttons: ['Tidak', 'Ya'] }).then(e => {
            if (e) {
                _delete(route('apbn-budgets.destroy', selectedItem[0]));
                setSelectedItem([]);
            };
        });
    };
    ...
    const searchData = useCallback(_.debounce(query => {
        Inertia.reload({ data: { query, page: 1 }, only: ['apbnBudgets'] });
    }, 300), []);
    const changePerPage = e => {
        const per_page = e.target.value;
        setPerPage(per_page);
        Inertia.reload({ data: { per_page, page: 1 }, only: ['apbnBudgets'], replace: true });
    };
    return (
        <Card title="Anggaran APBN">
            ...
            <ModalFade title={ `${formState == 'create' ? 'Tambah' : 'Edit'} Anggaran APBN` } ref={ modal }>
            ...
            <Table>
                <thead>
                    <tr>
                        <th>{ apbnBudgets.data.length > 0 && <input type="checkbox" title="Check ALL" onChange={ (e) => setSelectedItem(!e.target.checked ? [] : apbnBudgets.data.map(item => item.id)) } checked={ apbnBudgets.data.length == selectedItem.length } /> }</th>
                ...
                <tbody>
                    { apbnBudgets.data.length == 0 && <tr><td className="text-center h1" colSpan={ 999999 }>Data Not Available</td></tr> }
                    { apbnBudgets.data.map(regencyBudget => {
            ...
            <PaginateLinks links={ apbnBudgets.links } only={ ['apbnBudgets'] } />
