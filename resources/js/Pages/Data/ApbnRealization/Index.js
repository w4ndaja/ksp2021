...
export default function Index({ apbnRealizations, years, opdWorkUnits, request }) {
    ...
    const edit = e => {
        let { year_id, opd_work_unit_id, january, february, march, april, mei, june, july, august, september, october, november, december } = apbnRealizations.data.find(item => item.id == selectedItem[0] || 0);
        reset();
    ...
    const submitForm = e => {
        ...
        switch (formState) {
            case 'create':
                post(route('apbn-realizations.store'));
                break;
            case 'edit': patch(route('apbn-realizations.update', selectedItem[0])); break;
            case 'delete': _delete(route('apbn-realizations.destroy', selectedItem[0])); break;
        }
    };
    const destroy = e => {
        e.preventDefault();
        swal('Peringatan', 'Apakah anda yakin ingin menghapus data terpilih?', 'warning', { buttons: ['Tidak', 'Ya'] }).then(e => {
            if (e) {
                _delete(route('apbn-realizations.destroy', selectedItem[0]));
                setSelectedItem([]);
            };
        });
    };
    ...
    const searchData = useCallback(_.debounce(query => {
        Inertia.reload({ data: { query, page: 1 }, only: ['apbnRealizations'] });
    }, 300), []);
    const changePerPage = e => {
        const per_page = e.target.value;
        setPerPage(per_page);
        Inertia.reload({ data: { per_page, page: 1 }, only: ['apbnRealizations'], replace: true });
    };
    return (
        <Card title="Realisasi APBD">
            ...
            <Table>
                <thead>
                    <tr>
                        <th>{ apbnRealizations.data.length > 0 && <input type="checkbox" title="Check ALL" onChange={ (e) => setSelectedItem(!e.target.checked ? [] : apbnRealizations.data.map(item => item.id)) } checked={ apbnRealizations.data.length == selectedItem.length } /> }</th>
                ...
                <tbody>
                    { apbnRealizations.data.length == 0 && <tr><td className="text-center h1" colSpan={ 999999 }>Data Not Available</td></tr> }
                    { apbnRealizations.data.map(opdTarget => {
            ...
            <PaginateLinks links={ apbnRealizations.links } only={ ['apbnRealizations'] } />
        </Card>
    );
}
